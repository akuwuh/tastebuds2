...
>(function MicrophoneButton({ mode, onClick, className }, ref) {
  return (
    <motion.button
      ref={ref}
      onClick={onClick}
      className={`group relative flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-red-500 via-orange-500 to-yellow-500 text-white shadow-[0_20px_60px_rgba(255,71,87,0.6)] transition-transform hover:scale-105 active:scale-95 pulse-warm ${
        className ?? ""
      }`}
      whileTap={{ scale: 0.94 }}
    >
      <motion.div
        className="absolute inset-[-18%] rounded-full border border-orange-300/40"
        animate={
          rippleVariants[mode.toLowerCase() as keyof typeof rippleVariants] ??
          rippleVariants.idle
        }
      ...
      <span className="absolute top-[110%] w-28 text-center text-xs font-medium tracking-wide text-orange-200">
        {modeToLabel[mode]}
      </span>
    </motion.button>
  );
